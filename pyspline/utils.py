...
def point2Line(a: np.ndarray, vec: np.ndarray, p: np.ndarray) -> np.ndarray:
    t = np.dot(p - a, vec) / np.dot(vec, vec)
    return a + t * vec


def intersect3DLines(P0, T0, P1, T1):
    if np.all(np.cross(T0, T1) == 0):
        return 1, None, None, P1
