...
def trilinearVolume(*args):
    ...
    if len(args) == 1:
        # TODO: AL, Call signature is wrong and needs to be fixed once volumes are available
        return BSplineVolume(coef=args[0], tu=tu, tv=tv, tw=tw, ku=ku, kv=kv, kw=kw)
    elif len(args) == 2:
        ...
        return BSplineVolume(coef=coef, tu=tu, tv=tv, tw=tw, ku=ku, kv=kv, kw=kw)
    else:
...
def bilinearSurface(*args):
    ...
    2. ``p1``, ``p2``, ``p3``, ``p4``. Individual corner points in CCW Ordering::
          ...
          0          1
    """
    # TODO: AL, Make the arguments explicit and add types
    # TODO: AL, Fix the call signatures for the surfaces
    ...
    if len(args) == 1:
        ...
        coef[0, 1] = args[0][2]
        coef[1, 1] = args[0][3]
        return BSplineSurface(coef=coef, tu=[0, 0, 1, 1], tv=[0, 0, 1, 1], ku=2, kv=2)
    else:
        # Assume 4 arguments
        coef = np.zeros([2, 2, 3])
        coef[0, 0] = args[0]
...
def line(*args, **kwargs):
    ...
    """
    # TODO: AL, Fix this to work correctly with the new BSpline interface
    # TODO: AL, Make the arguments explicit and add types
    if len(args) == 2:
        # Its a two-point type
        return BSplineCurve(coef=[args[0], args[1]], k=2, t=[0, 0, 1, 1])
    elif len(args) == 1:
        if len(args[0]) == 2:  # its X
            return BSplineCurve(coef=args[0], k=2, t=[0, 0, 1, 1])
        elif "dir" in kwargs:
            ...
            return BSplineCurve(coef=[args[0], x2], k=2, t=[0, 0, 1, 1])
        else:
